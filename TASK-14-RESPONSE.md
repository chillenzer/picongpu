...
| m2      | accepted - fixed | `ef4df0d3f` | §7.1 interim-fallback description corrected: the external wrapper yields only workflow-level events (it cannot delimit steps, since `submission_information.txt` is visible in the run dir only as the last step's output); M2 step events require the hook or option (i). |
...
| n1      | accepted - fixed | `20c730b7e` | In mcp 2.1.1 the decorator is the `server.tool()` instance method (`mcp/server/mcpserver/server.py:654`), not a module-level `mcp.tool()`; `MCPServer` is at `server.py:153`. Fixed §4 and Appendix A2. |
