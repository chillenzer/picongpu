...
class ThermalizationVerifier:
    ...
    def __init__(self, sim_output_path):
        ...
        self.e_T_std_mean = np.zeros(len(self.series.iterations), dtype=np.float64)
        self.i_T_std_mean = np.zeros(len(self.series.iterations), dtype=np.float64)
        self.e_T_std_dist = np.zeros(len(self.series.iterations), dtype=np.float64)
        self.i_T_std_dist = np.zeros(len(self.series.iterations), dtype=np.float64)
    ...
    def plot(self, to_file=False, file_name=None, smilei_sim=None):
        ...
        times = (
            np.array(self.series.iterations)
            * self.series.iterations[0].dt
            * self.series.iterations[0].time_unit_SI
            * 1e15
        )
        ...
        if smilei_sim is not None:
            ...
            for ax in [ax_dict["main"], ax_dict["zoom"]]:
                ...
                ax.fill_between(
                    smilei_sim.times,
                    smilei_sim.e_T_mean - 2 * smilei_sim.e_T_std_mean,
                    smilei_sim.e_T_mean + 2 * smilei_sim.e_T_std_mean,
                    alpha=0.1,
                    color=color_electrons,
                )
                ...
                ax.fill_between(
                    smilei_sim.times,
                    smilei_sim.i_T_mean - 2 * smilei_sim.i_T_std_mean,
                    smilei_sim.i_T_mean + 2 * smilei_sim.i_T_std_mean,
                    alpha=0.1,
                    color=color_ions,
                )
        ...
        for ax in [ax_dict["main"], ax_dict["zoom"]]:
            ...
            ax.fill_between(
                times,
                self.e_T_mean - 2 * self.e_T_std_mean,
                self.e_T_mean + 2 * self.e_T_std_mean,
                alpha=0.1,
                color=color_electrons,
            )
            ...
            ax.fill_between(
                times,
                self.i_T_mean - 2 * self.i_T_std_mean,
                self.i_T_mean + 2 * self.i_T_std_mean,
                alpha=0.1,
                color=color_ions,
            )
            ...
            ax.plot(
                times,
                (self.e_T_mean + self.i_T_mean) / 2,
                label=r"$(T_e + T_i)/2$ PIConGPU",
                linestyle="dotted",
                color=color_mean,
            )
        ax = ax_dict["std_dist"]
        ax.plot(
            times,
            self.e_T_mean,
            label=r"$T_e$ PIConGPU $\pm 2\sigma$",
            color=color_electrons,
        )
        ax.fill_between(
            times,
            self.e_T_mean - 2 * self.e_T_std_dist,
            self.e_T_mean + 2 * self.e_T_std_dist,
            alpha=0.1,
            color=color_electrons,
        )
        ax.plot(
            times,
            self.i_T_mean,
            label=r"$T_i$ PIConGPU $\pm 2\sigma$",
            color=color_ions,
        )
        ax.fill_between(
            times,
            self.i_T_mean - 2 * self.i_T_std_dist,
            self.i_T_mean + 2 * self.i_T_std_dist,
            alpha=0.1,
            color=color_ions,
        )
    ...
    def plot_debug_values(self, to_file=False, file_name=None, smilei_loader=None):
        ...
        line_style = "-"
        times = self.debug_values["ii"]["iteration"] * self.dt * 1e15
        for ii, collision_pair in enumerate(self.pairs):
            axs[ii][0].plot(
                times,
                self.debug_values[collision_pair]["coulomb_log"],
                label=rf"$\ln\Lambda$ PIConGPU {collision_pair}",
                color="red",
                ls=line_style,
                alpha=0.75,
            )
            axs[ii][1].plot(
                ...
                label=rf"$s$ PIConGPU {collision_pair}",
                color="red",
                ls=line_style,
                alpha=0.75,
            )
            if smilei_loader is not None:
                ...
                axs[ii][0].plot(
                    times_smilei,
                    (smilei_loader.debug_values[collision_pair]["coulomb_log"]),
                    label=rf"$\ln\Lambda$ smilei {collision_pair}",
                    color="black",
                    ls=line_style,
                    alpha=0.75,
                )
                axs[ii][1].plot(
                    times_smilei,
                    (smilei_loader.debug_values[collision_pair]["s_param"]),
                    label=rf"$s$ smilei {collision_pair}",
                    color="black",
                    ls=line_style,
                    alpha=0.75,
                )
        ...
        axs[0][0].plot(times, self.coulomb_log, label="coulomb log theory ei")
        ...
        if self.average_debye_present:
            ...
            if smilei_loader is not None:
                ...
                axs[2][2].plot(
                    times_smilei,
                    (smilei_loader.debug_values["ee"]["debye_length"]),
