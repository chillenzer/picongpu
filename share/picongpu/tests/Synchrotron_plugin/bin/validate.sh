...
if [ ! -z $picongpuPrefix ]; then
    if [ $test_return -ne 0 ]; then
        echo -e $testSuite`echo_r "Synchrotron radiation plugin has bad performance!"`
        echo -e $testSuite`echo_r "Check Synchrotron plugin implementation!"`
        echo -e $testSuite"`thumbs_down`"
    else
        echo -e $testSuite`echo_g "Synchrotron plugin implementation successfully validated!"`
