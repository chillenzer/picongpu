...
- pypicongpu:
    ...
    - fix the ionization name/current bridging between the PICMI and pypicongpu layers: the PICMI field-ionization models (ADK, BSI, Keldysh) previously hard-coded the pypicongpu `None_` ionization current and silently dropped a user-provided `ionization_current`; the current is now bridged (None maps to `None_`, a concrete current must convert to a pypicongpu ionization current model or is rejected with a clear error instead of being silently dropped, which would change the physics). The PICMI `ThomasFermi` conversion was broken (it constructed the pypicongpu model without the required electron species, which the C++ `ThomasFermi` ionizer is parameterised with) and now bridges `ionization_electron_species`. The pypicongpu `ThomasFermi` model no longer accepts an ionization current, matching the C++ interface (the byCollision `ThomasFermi<T_DestSpecies>` ionizer takes no ionization-current template argument, unlike the byField ionizers). The pypicongpu `ionizer_picongpu_name` values (BSI, BSIEffectiveZ, BSIStarkShifted, ADKLinPol, ADKCircPol, Keldysh, ThomasFermi) were verified against the C++ ionizer type names. No change to valid inputs; rendered output is byte-identical.
    - make the pypicongpu collision model the single source of truth for collision semantics: the invariant "a dynamic-log collision (Coulomb logarithm computed from the Debye screening length) requires at least one screening species" (a documented C++ requirement) is now enforced by `pypicongpu.collisions.CollisionalPhysicsSetup` — alongside the other collision invariants — instead of by the PICMI layer. The PICMI `Collision`/`CollisionalPhysicsSetup` are now thin bridges that hold the PICMI species until conversion and delegate validation/serialisation/rendering to their pypicongpu counterparts (this moves that specific error from PICMI-construction time to conversion time; the message is unchanged in substance). No change to valid inputs; serialisation and rendered output are byte-identical.
    - distinguish the charge state from the bound-electron count, as in the C++ interface: the `SetChargeState` operation now documents its actual C++ counterpart (the `manipulators::unary::ChargeState<T_chargeState>` init-pipeline manipulator) and the distinction between the charge state (number of stripped electrons, the C++ template parameter) and the bound-electron count (the `boundElectrons` species attribute = atomic number - charge state, which C++ derives and stores). The `SetChargeState` operation additionally rejects unphysical charge states at construction time (charge state > atomic number), mirroring the C++ compile-time assertion `Too_high_charge_state_for_atomic_number`, whenever the species carries the `ElementProperties` constant (otherwise the C++ compile-time check still applies). The `BoundElectrons` attribute docstring and the PICMI `Species.charge_state` attribute are documented accordingly. No change to valid inputs; rendered output is byte-identical.
    - align the density-ratio semantics with the C++ interface: the `Gaussian` density profile's plateau field is renamed `density` to `density_si` (matching `Uniform`/`Foil`/`Cylinder` and the SI unit policy; the PICMI attribute name `density` is kept as an alias, so the `picmi` bridge and existing call sites are unchanged). The `DensityRatio` docstring now documents the actual C++ semantics (`DensityRatio_<typename>` is a runtime `value_identifier` passed as the `densityRatio<>` frame flag and read via `traits::GetDensityRatio`; a species without it falls back to the C++ default of 1.0, and species derived inside a `CreateDensity` are scaled by the ratio of their density ratios). Density profiles stay stored in absolute SI and render the normalising expression `<density_si> / SI::BASE_DENSITY_SI` (the C++ base density is a user-configurable simulation parameter, unlike the fixed mass/charge base constants, so the normalisation is left to the C++ compile time). Rendered `.param`/`.cfg` output is byte-identical; only the metadata JSON key of the Gaussian profile changes (`density` -> `density_si`).
    - mirror the C++ species mass/charge ratio concept (`MassRatio_<typename>`/`ChargeRatio_<typename>` in `speciesDefinition.param`, relative to `SI::BASE_MASS_SI`/`SI::BASE_CHARGE_SI` in `speciesConstants.param`): the stored SI values (`Mass.mass_si`, `Charge.charge_si`) are unchanged, but `Mass`/`Charge` now expose a computed `mass_ratio`/`charge_ratio` (dimensionless, relative to the new `SpeciesConstants` base values, which mirror the C++ fixed electron mass/charge), and `speciesDefinition.param` renders the precomputed ratio directly instead of the compile-time expression `<si> / sim.si.getBaseMass()`. The rendered value is numerically identical (same IEEE double C++ computed); the template file now only contains the ratio, exactly like the C++ default `speciesDefinition.param`. All other rendered `.param`/`.cfg` files are byte-identical.
