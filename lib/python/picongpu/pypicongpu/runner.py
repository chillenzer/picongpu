...
class Stage(str, enum.Enum):
    ...
    Stages are thin aliases over the steps of ``workflow.cwl`` and the
    workflow's top-level outputs that expose what each stage produces. The
    Python package only uses them to *generate the right in-process cwltool
    invocation* (see :meth:`Runner.run_invocation`); all actual workflow logic
    -- wiring, skipping completed steps, resuming -- is done by cwltool
    itself (via ``--target`` and the job store), not reimplemented here.

    The stages, in execution (topological) order, are:
...
# The top-level workflow outputs each stage produces (declared in
# templates/workflow/workflow.cwl). Asking cwltool for the outputs of a stage
# (via ``--target``) makes it run exactly the steps upstream of them and prune
# the downstream stages -- that is how a partial (prefix) run is generated.
# These names are a stable alias over the workflow outputs; if the workflow is
# reorganized, only this mapping (and the workflow itself) change.
STAGE_OUTPUTS: dict[Stage, tuple[str, ...]] = {
    Stage.build: ("build_bin_directory",),
    Stage.prepare: ("prepare_tbg_directory",),
    Stage.submit: (
        "submit_submission_information",
        "submit_link_results_script",
        "submit_tbg_directory",
    ),
    # The full-pipeline outputs (as produced by the organize_output step):
    # this is also what the default run asks for, so the full run behaves
    # exactly as before (the per-stage outputs above are never collected
    # unless a prefix run targets them).
    Stage.collect: (
        "input_directory",
        "tbg_directory",
...
class CwltoolInvocation:
    """
    In-process cwltool invocation generated by :meth:`Runner.run_invocation`.

    Thin binding between the stable stage vocabulary and cwltool's in-process
    ``WorkflowFactory`` (``cwltool.factory.Factory``): it carries *what* to run
    (workflow, job order file, run and cache directory) and *which* top-level
    outputs to request (``targets``, the stage -> output aliases from
    :data:`STAGE_OUTPUTS`), and :meth:`run` executes it in the **current
    process** -- no subprocess is spawned. All workflow logic (wiring,
    skipping completed steps, resuming) stays in cwltool; nothing is
    reimplemented here.
    """
    ...
    @property
    def target_outputs(self) -> tuple[str, ...]:
        """The top-level workflow outputs this invocation requests (cwltool's ``--target``)."""
        return self.targets

    def run(self) -> dict:
        """
        Execute the (requested subset of the) workflow in-process.

        The workflow is loaded via cwltool's ``WorkflowFactory`` and executed
        in the same process. If :attr:`targets` are set, the workflow is first
        reduced to the subgraph of exactly those top-level outputs (cwltool's
        ``--target``), so only the steps contributing to them are executed;
        without targets the full workflow runs (against the job store, which
        is what implements ``from_`` resuming).
        """
        cachedir = os.path.abspath(self.cachedir)
        ...
        runtime_context.tmp_outdir_prefix = cachedir + "_tmp"
        os.makedirs(cachedir, exist_ok=True)

        loading_context = LoadingContext()
        loading_context.loader = default_loader()
        factory = WorkflowFactory(runtime_context=runtime_context, loading_context=loading_context)

        callable_tool = factory.make(str(self.workflow_path))
        if self.targets:
            tool = callable_tool.t
            target_ids = [loading_context.loader.fetcher.urljoin(tool.tool["id"], "#" + name) for name in self.targets]
            callable_tool = factory.make(get_subgraph(target_ids, tool, loading_context))

        with self.input_path.open() as file:
            job_order = json.load(file)
        try:
            return callable_tool(**job_order)
        except WorkflowStatus as exc:
            raise RuntimeError("cwltool failed (status {}):\n{}".format(exc.status, exc.out)) from exc
...
class Runner(BaseModel):
    ...
    **If those sanity checks pass, the respective process is launched.**
    ...
    def run(self, up_to: Stage | str | None = None, from_: Stage | str | None = None) -> dict:
        """
        Run (the requested subset of) the compiled picongpu workflow.

        Partial execution is delegated to cwltool, invoked in-process through
        its ``WorkflowFactory`` (see :meth:`run_invocation`): the workflow is
        always submitted in full, but

        - ``up_to`` prunes every step after the requested stage (cwltool only
          executes the steps contributing to the requested outputs), and
        - ``from_`` resumes at the requested stage: cwltool serves the
          earlier steps from the job store (``<run_dir>/.cwl_cache``) because
          their inputs are byte-identical, and recomputes the requested stage
          and everything after it.

        The job store is the only state cwltool keeps between invocations; the
        runner itself writes none.

        Returns the final workflow outputs as a dict.
        """
        return self.run_invocation(up_to=up_to, from_=from_).run()

    def run_invocation(self, up_to: Stage | str | None = None, from_: Stage | str | None = None) -> CwltoolInvocation:
        """
        Generate the in-process cwltool invocation for (the requested subset of) the workflow.

        Partial execution is done entirely by cwltool:

        - ``up_to``: cwltool is given the full workflow but asked to produce
          the top-level outputs of the requested stage as its final outputs
          (cwltool's ``--target``, see :data:`STAGE_OUTPUTS`). It then
          executes exactly the steps that contribute to those outputs and
          prunes the downstream steps, i.e. everything after the stage is not
          run at all.
        - ``from_``: the full workflow is run against the persistent cwltool
          job store (``--cachedir``); steps whose inputs are byte-identical
          to an earlier run (everything before the resume point) are served
          from the store, everything from the requested stage on recomputes.
          This is how an interrupted run is resumed without redoing the
          completed stages.
        - neither: the full workflow is run, exactly as before.

        The two selectors are mutually exclusive.

        The invocation is executed in-process via cwltool's ``WorkflowFactory``;
        no separate cwltool subprocess is started.
        """
        if up_to is not None and from_ is not None:
            raise ValueError(
                "cannot combine up_to and from_ in a single run: up_to selects a prefix of the "
                "workflow, from_ resumes from a stage (use one of them)"
            )

        stage = Stage(up_to) if up_to is not None else Stage.collect
        return CwltoolInvocation(
            workflow_path=self.workflow_definition_path,
            input_path=self.workflow_input_path,
            outdir=self.run_dir,
            cachedir=self.cwl_cachedir,
            targets=STAGE_OUTPUTS[stage],
        )
