...
class Stage(str, enum.Enum):
    ...
    Python package only uses them to *generate the right cwltool invocation*
    (see :meth:`Runner.run_command`); all actual workflow logic -- wiring,
    skipping completed steps, resuming -- is done by cwltool itself (via
    ``--target`` and the job store), not reimplemented here.
...
class Runner(BaseModel):
    ...
    **If those sanity checks pass, the respective process is launched.**
    ...
    def run(self, up_to: Stage | str | None = None, from_: Stage | str | None = None):
        """
        run (the requested subset of) the compiled picongpu workflow

        Partial execution is delegated to cwltool, which is invoked with the
        command line generated by :meth:`run_command`: the workflow is always
        submitted in full, but
        ...
        """
        command = self.run_command(up_to=up_to, from_=from_)
        logging.info("running cwltool: %s", " ".join(command))
        result = subprocess.run(command, text=True, capture_output=True)
        if result.returncode != 0:
            raise RuntimeError("cwltool failed (exit code {}):\n{}".format(result.returncode, result.stderr))
        # cwltool writes the final outputs JSON to stdout; its logs go to stderr.
        return json.loads(result.stdout)

    def run_command(self, up_to: Stage | str | None = None, from_: Stage | str | None = None) -> list[str]:
        """
        Generate the cwltool invocation that executes (the requested subset of) the workflow.

        Partial execution is done entirely by cwltool:

        - ``up_to``: cwltool is given the full workflow but ``--target`` set
          to the top-level outputs of the requested stage (see
          :data:`STAGE_OUTPUTS`). cwltool then executes exactly the steps
          that contribute to those outputs and prunes the downstream steps,
          i.e. everything after the stage is not run at all.
        ...
        command = [
            ...
            "--copy-outputs",
            "--cachedir",
            str(self.cwl_cachedir),
            "--preserve-entire-environment",
        ]
        stage = Stage(up_to) if up_to is not None else Stage.collect
        for output_name in STAGE_OUTPUTS[stage]:
            command += ["--target", output_name]
        command += [str(self.workflow_definition_path), str(self.workflow_input_path)]
        return command
