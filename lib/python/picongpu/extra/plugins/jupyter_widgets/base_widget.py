...
class BaseWidget(widgets.VBox):
    ...
    def set_run_dir_options(self, run_dir_options):
        ...
        self.plot_mpl = plot_mpl_class(run_directories=None, ax=self.ax)
    ...
    def _init_fig_and_ax(self, fig, **kwargs):
        ...
        if fig is None:
            ...
            lab = str(type(self)) + "_" + datetime.now().strftime("%Y%m%d%H%M%S")
    ...
    def _get_widget_args(self):
        ...
        """
        return {arg: widg.value for arg, widg in self.widgets_for_vis_args.items()}
