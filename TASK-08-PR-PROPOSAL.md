...
Branch: `task-08-ruff-all` (30 commits on `dev` = b4e4ca5b2)
Footprint: 240 files, +2280 / −2066 — 239 files are auto-fixes + small manual
fixes; the substantive decisions are all in `ruff.toml`.
...
`ruff.toml` as the single source of truth and tightens the rule set in 29
incremental steps (one commit per rule batch, each independently green) up
to `select = ["ALL"]` — the complete stable ruff rule set.
...
The 30 commits map 1:1 to the spec's incremental steps and can be merged as
one PR or squashed per batch. Suggested PR description per batch is in each
commit message; the full exception list lives in `ruff.toml` comments.
...
| 3 | cb95ff2c9 | I (isort) | 181 import-block fixes across ~86 files |
...
| 19 | 89d26a20a | S (bandit) | 36 S101 `assert`→`AssertionError` in library code; S110 restructure; per-file-ignores for test/example/tool trees |
...
- **Justified family ignores** (each with a one-line+ justification in
  `ruff.toml`):
  - `D` (pydocstyle, ~4.8k): docstrings deliberately mix conventions
    ...
    project.
  - `ANN` (~3.5k): full annotation coverage is a separate typing project;
    public API boundaries are runtime-guarded with `typeguard`.
  - `TRY`/`EM` (~600): the codebase raises built-in exceptions with inline
  ...
  - `PTH` (~110): `os.path` is the consistent style in scripts/tools;
    pathlib migration is a separate modernisation.
  ...
  - `TID252` (78): relative imports are the package convention (TID251 is
...
- **Kept from the incremental batches** (still needed, audited): E721,
  PLR0911/0912/0913/0915, PLR2004, PT009/011/012/027.
...
1. **`particle_functor.py` — `NotImplementedError()` without `raise`**
   ...
   NotImplementedError(...)`.
2. **`S110`** (bandit, batch 19): a try/except with a bare `except:` was
   restructured to catch the specific exception.
3. **S101 in library code** (batch 19): 36 `assert`s converted to `raise
   AssertionError(...)` — *intentionally* changes `python -O` behaviour
