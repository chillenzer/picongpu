...
| CI job dry-run (re-run, rework) | `share/ci/docs_snippets_dryrun.sh` - the job's `script:` entries as YAML-parsed (exactly what the GitLab runner executes) run verbatim with the task venv; stubbed only: `pypicongpu.sh` (micromamba env setup), `apt`, the `pic-build`/`pic-configure`/`tbg` `--help` smoke checks | **all 16 job steps passed** at the final rework tip (~4 min, 2026-08-31): snippet suite `25 passed in 29.82s`; `profile check OK: PIC_BACKEND=omp2b:native PICSRC=<source tree>`; doxygen + `sphinx-build`: `build succeeded` (412 warnings in this run's environment - the absolute count is environment/build-state dependent: 381 in the original local build, 283-413 across the re-run dry runs, 412 at the final tip; **0** in `python_package/` and **0** include failures in every run); both grep gates passed |
