...
# profile's variables ($ROSI_LIB, $MPI_CXX, ...).

if [ -z "${PIC_PROFILE:-}" ] || [ ! -f "${PIC_PROFILE:-}" ]; then
    printf 'Set PIC_PROFILE to your PIConGPU profile (e.g. etc/picongpu/rosi-hzdr/gpu-v100_picongpu.profile.example)!\n'
    exit 1
fi
...
# The old rosi script pointed ADIOS2/openPMD at this MPI library:
