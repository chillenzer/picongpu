...
- **Simulation**
  ...
  - ``picongpu_run(up_to=..., from_=...)``:
    build and run the simulation, optionally executing only a subset of the
    workflow stages (build/prepare/submit/collect) via cwltool's partial
    execution and its job store, see :ref:`Partial Workflow Execution (Stages) <picmi-partial-workflow>`.
